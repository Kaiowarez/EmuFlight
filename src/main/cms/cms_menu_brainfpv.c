...
osdConfig_t osdConfigCms;

static long menuBrainFPVOnEnter(void)
{
    memcpy(&osdConfigCms, osdConfig(), sizeof(osdConfig_t));
    return 0;
}
...
static long menuBrainFPVOnExit(const OSD_Entry *self)
{
    ...
    memcpy(osdConfigMutable(), &osdConfigCms, sizeof(osdConfig_t));
    return 0;
}

OSD_UINT8_t entryAhiSteps =  {&osdConfigCms.ahi_steps, 0, 9, 1};
const char *STICKS_DISPLAY_NAMES[] = {"OFF", "MODE2", "MODE1"};
OSD_TAB_t entrySticksDisplay = {&osdConfigCms.sticks_display, 2, &STICKS_DISPLAY_NAMES[0]};
const char *FONT_NAMES[] = {"DEFAULT", "LARGE", "BOLD"};
OSD_TAB_t entryOSDFont = {&osdConfigCms.font, 2, &FONT_NAMES[0]};
OSD_UINT8_t entryWhiteLevel =  {&osdConfigCms.white_level, 100, 120, 1};
OSD_UINT8_t entryBlackLevel =  {&osdConfigCms.black_level, 15, 40, 1};
OSD_UINT8_t entrySyncTh =  {&osdConfigCms.sync_threshold, BRAINFPV_OSD_SYNC_TH_MIN, BRAINFPV_OSD_SYNC_TH_MAX, 1};
OSD_INT8_t entryXoff =  {&osdConfigCms.x_offset, -8, 7, 1};
OSD_UINT8_t entryXScale =  {&osdConfigCms.x_scale, 0, 15, 1};
OSD_UINT8_t entry3DShift =  {&osdConfigCms.sbs_3d_right_eye_offset, 10, 40, 1};
OSD_UINT16_t entryMapMaxDist =  {&osdConfigCms.map_max_dist_m, 10, 32767, 10};

OSD_Entry cmsx_menuBrainFPVOsdEntries[] =
{
    {"-- BRAIN OSD --------", OME_Label, NULL, NULL, 0},
    {"AHI STEPS", OME_UINT8, NULL, &entryAhiSteps, 0},
    {"ALTITUDE SCALE", OME_Bool, NULL, &osdConfigCms.altitude_scale, 0},
    {"SPEED SCALE", OME_Bool, NULL, &osdConfigCms.speed_scale, 0},
    {"MAP", OME_Bool, NULL, &osdConfigCms.map, 0},
...
OSD_TAB_t entryHdFrameMode = {&osdConfigCms.hd_frame, 2, &HD_FRAME_NAMES[0]};
OSD_UINT8_t entryHdFrameWidth = {&osdConfigCms.hd_frame_width, 20, 255, 1};
OSD_UINT8_t entryHdFrameHeight = {&osdConfigCms.hd_frame_height, 20, 255, 1};
OSD_INT8_t entryHdFrameHOffset = {&osdConfigCms.hd_frame_h_offset, -100, 100, 1};
OSD_INT8_t entryHdFrameVOffset = {&osdConfigCms.hd_frame_v_offset, -100, 100, 1};
...
OSD_TAB_t entryIRSys = {&osdConfigCms.ir_system, 2, &IR_NAMES[0]};
OSD_UINT32_t entryIRIlap =  {&osdConfigCms.ir_ilap_id, 0, 9999999, 1};
OSD_UINT16_t entryIRTrackmate =  {&osdConfigCms.ir_trackmate_id, 0, 4095, 1};
...
OSD_TAB_t entryCrsfRssiMode = {&osdConfigCms.crsf_link_stats_rssi, 3, &CRSF_OPT_NAMES[0]};
OSD_TAB_t entryCrsfSnrMode = {&osdConfigCms.crsf_link_stats_snr, 3, &CRSF_OPT_NAMES[0]};
OSD_INT8_t entryCrsfSnrThreshold = {&osdConfigCms.crsf_link_stats_snr_threshold, -10, 10, 1};

OSD_Entry cmsx_menuBrainFPVCrsfLinkEntries[] =
{
    {"-- CRSF LINK QUALITY --", OME_Label, NULL, NULL, 0},

    {"ENABLED", OME_Bool, NULL, &osdConfigCms.crsf_link_stats, 0},
    {"TX POWER", OME_Bool, NULL, &osdConfigCms.crsf_link_stats_power, 0},
...
OSD_Entry cmsx_menuBrainFPVEntires[] =
{
    ...
    {"SPECTROGRAPH", OME_Bool, NULL, &osdConfigCms.spec_enabled, 0},
#endif /* defined(USE_BRAINFPV_SPECTROGRAPH) */
    {"SHOW LOGO ON ARM", OME_Bool, NULL, &osdConfigCms.show_logo_on_arm, 0},
    {"SHOW PILOT LOGO", OME_Bool, NULL, &osdConfigCms.show_pilot_logo, 0},
    {"BACK", OME_Back, NULL, NULL, 0},
    {NULL, OME_END, NULL, NULL, 0}
};
